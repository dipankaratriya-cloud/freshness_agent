...
echo "   bq_io.py reads datcom-store.spanner_dc_graph_prod_DEFAULT.{Edge,Node},"
...
echo ""
echo "⚠  Tier 2 (download + pi coding agent) needs the pi CLI's non-interactive"
echo "   Google-provider auth to actually work in this container — it reads"
echo "   GEMINI_API_KEY straight from the environment (already injected via"
echo "   --set-secrets in cloudbuild.yaml), confirmed with a live local smoke"
echo "   test of the pi CLI itself. No further auth step should be needed, but"
echo "   this has NOT yet been confirmed inside an actual Cloud Run execution —"
echo "   verify on the first real run before trusting Tier 2 results from this job."
echo ""
echo "   Also consider adding TIER1_CONCURRENCY=3 to cloudbuild.yaml's"
echo "   --set-env-vars (not currently there) — the code's local-machine"
echo "   default of 6 concurrent Chromium+Gemini sessions may be too many for"
echo "   this job's 8Gi/4vCPU allocation."
